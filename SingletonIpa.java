...
public class SingletonIpa {
		...
		Been been=null;
		 for(int i=0;i<datas.size();i++){
    		 Been target=datas.get(i);
    		  if(userId.equals(target.userId)  && projectId.equals(target.getProjectId())){
    			 been=target;
    			 break;
    		  }
    ...
    public void find(Been src){
    	 for(int i=0;i<datas.size();i++){
    		 Been target=datas.get(i);
    		  if(src.getUserId().equals(target.userId)  && src.getProjectId().equals(target.getProjectId())){
    			  
    			  System.out.println("src name:"+src.displayName +" targetName:"+target.displayName);
    		  }
